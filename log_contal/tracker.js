(function () {
    /**
     * 负责读取cookie数据
     * */

    var cookieUtils = {
        //向浏览器cookie写入数据
        set: function (cookieName, cookieValue) {
            var coookieText = encodeURIComponent(cookieName) + "=" + encodeURIComponent(cookieValue);
            //计算出十年的时间
            var date = new Date();
            //获取当前的时间戳
            var currenTime = date.getTime();
            //获取十年的长度
            var tenyearTime = 10 * 365 * 24 * 60 * 60 * 1000;
            //十年后的时间
            var tenTime = currenTime + tenyearTime;
            //将date时间调整为10年后的时间
            date.setTime(tenTime);
            //设置过期时间
            coookieText += ";expires=" + date.toUTCString();
            document.cookie = coookieText
        },

        get: function (cookieName) {
            //向浏览器获取cookie的数据
            var cookievalue = null;
            var cookieText = document.cookie;//uuid=123;sid=456;mid=789
            if (cookieText.indexOf(encodeURIComponent(cookieName)) > -1) {
                var items = cookieText.split(";");
                for (index in items) {
                    var kv = items[index].split("=");
                    var key = kv[0].trim();
                    var value = kv[1].trim();
                    if (key == encodeURIComponent(cookieName)) {
                        cookievalue = decodeURIComponent(value)
                    }
                }
            }
            return cookievalue;
        }
    };

    /**
     * 负责具体采集每个用户的行为事件数据
     * */

    var tarck = {
        clientConfig: {
            logServerUrl: "http://NameNode/log.gif",//日志服务器地址
            sessionTimeOut: 2 * 60 * 1000,//会话过期时间
            logVersion: "1.0"
        },

        /**
         * 需要存到cookie中的字段
         * */
        cookieKeys: {
            uuid: "uid",//用户的唯一标识
            sid: "sid",//会话标识
            preVisitTime: "preVisitTime"//用户最近一次访问时间
        },

        /**
         * 定义需要采集的事件名称（根据业务来）
         * */
        events: {
            launchEvent: "e_l",//用户首次访问事件     自动触发
            pageViewEvent: "e_pv",//用户浏览页面事件  自动触发
            addCartEvent: "e_ad",//商品加入购物车事件 手动触发
            searchEvent: "e_s"//搜索事件              手动触发
        },

        /**
         * 定义需要采集的字段（这个一定是根据具体业务来）
         */
        columns: {
            eventName: "en",//事件名称
            version: "ver",//日志版本
            platform: "pl",//平台 ios Android
            sdk: "sdk",//sdk js java
            uuid: "uid",//用户唯一标识
            sessionId: "sid",//会话id
            resolution: "b_rst",//浏览器分辨率
            userAgent: "b_usa",//浏览器代理信息
            language: "l",//语言
            clientTime: "ct",//客户端时间
            currentUrl: "url",//当前页面的url
            referrerUrl: "ref",//来源url，上一个页面的url
            title: "tt",//网页标题
            keyword: "kw",//搜索关键字
            goodsId: "gid"//商品id
        },

        /**
         * 设置uid到cookie
         * */
        setuid: function (uuid) {
            cookieUtils.set(this.cookieKeys.uuid, uuid);
        },
        //获取uuid
        getuid: function () {
            return cookieUtils.get(this.cookieKeys.uuid);
        },

        //设置sid到cookie
        setsid: function (sid) {
            cookieUtils.set(this.cookieKeys.sid, sid);
        },
        //获取sid
        getsid: function () {
            cookieUtils.get(this.cookieKeys.sid);
        },

        //会话开始了
        sessionStart: function () {
            //判断回话是否存在
            if (!this.getsid()) {//会话不存在
                //创建会话
                this.creatsession();
            } else {//会话存在
                //判断会话是否过期
                if (this.isSessionTimeOut()) {//会话过期了，创建会话
                    this.creatsession();
                } else {//没有过期
                    this.updateTime();//更新用户最近一次访问时间
                }
            }
            //触发pageViewEvent事件
            this.pageviewEvent();
        },

        /**
         * 创建用户浏览页面事件
         * */
        pageviewEvent: function () {
            var data = {};
            //事件名称
            data[this.columns.eventName] = this.events.pageViewEvent;
            //设置公共字段
            this.setCommonColumns(data);
            //当前页面的url
            data[this.columns.currentUrl] = window.location.href;
            //当前页面的标题
            data[this.columns.title] = document.title;
            //来源页面的url
            data[this.columns.referrerUrl] = document.referrer;
            //发送数据到日志服务器上
            this.sendDataToLogServer(data);
        },

        /**
         * 创建商品加入购物车事件
         * */
        addCartEvent: function (goosId) {
            var data = {};
            //事件名称
            data[this.columns.eventName] = this.events.addCartEvent;
            //公共字段
            this.setCommonColumns(data);
            //商品加入商品购物车根据商品id添加
            data[this.columns.goodsId] = goosId;
            //发送数据到服务器
            this.sendDataToLogServer(data);
        },

        /**
         * 创建搜索事件
         * */
        searchEvent: function (keyword) {
            var data = {};
            data[this.columns.eventName] = this.events.searchEvent;
            this.setCommonColumns(data);
            data[this.columns.keyword] = keyword;
            this.sendDataToLogServer(data);
        },


        /**
         * 创建新的会话
         * */
        creatsession: function () {
            //生成会话id
            var sessionid = this.guid();
            //将回话保存到cookie中
            this.setsid(sessionid);
            //判断用户是否是首次访问(查看浏览器的cookie中是否用用户的唯一标识，如果有说明是非首次访问，否则是首次访问）
            if (!this.getuid()) {//首次访问
                var uid = this.guid();//生成用户id
                this.setuid(uid);//将用户id保存到cookie中
                //触发首次访问事件
                this.launchEvent();
            }
        },

        /**
         * 判断会话是否过期
         * */
        isSessionTimeOut: function () {
            //获取当前时间
            var date = new Date().getTime();
            //获取用户最近一次访问时间
            var preVisitTime = cookieUtils.get(this.cookieKeys.preVisitTime);
            return date - preVisitTime > this.clientConfig.sessionTimeOut;
        },


        /**
         * 定义首次访问事件
         * */
        launchEvent: function () {
            //定义一个对象，这个对象用来封装收集好的数据
            var data = {};
            //事件名称 data["en"]="e_l"
            data[this.columns.eventName] = this.events.launchEvent;
            //设置公共字段
            this.setCommonColumns(data);
            //将收集好的数据发送到日志服务器上
            this.sendDataToLogServer(data);
        },


        /**
         * 将数据发送到日志服务器上去
         * */
        sendDataToLogServer: function (data) {
            ////data==> key value==> key=value&key=value&....
            var paramtext = "";
            for (key in data) {
                if (key && data[key]) {
                    paramtext = encodeURIComponent(key) + "=" + encodeURIComponent(data[key]) + "&"
                }
                if (paramtext)
                    paramtext = paramtext.substr(0, paramtext.length - 1);
                var url = this.clientConfig.logServerUrl + "?" + paramtext;
                var i = new Image(1, 1);
                i.src = url;
                //更新用户最近一次访问时间
                this.updateTime();
            }
        },
        //用户最近访问时间
        updateTime: function () {
            cookieUtils.set(this.cookieKeys.preVisitTime, new Date().getTime());
        },

        /**
         * 设置公共字段
         * */
        setCommonColumns: function (data) {
            //版本号
            data[this.columns.version] = this.clientConfig.logVersion;
            //用户代理信息
            var userAgent = window.navigator.userAgent.toLowerCase();
            data[this.columns.userAgent] = userAgent;
            /**
             * indexOf 用来查找子字符串是否在指定字符串之内，如果存在，返回对应字符串起始角标，否则返回 -1
             */
            if (userAgent.indexOf("android") > -1) {
                data[this.columns.platform] = "android";
            } else if (userAgent.indexOf("phone") > -1) {
                data[this.columns.platform] = "ios";
            } else {
                data[this.columns.platform] = "pc";
            }
            data[this.columns.sdk] = "js";
            //用户唯一标识
            data[this.columns.uuid] = this.getuid();
            //会话id
            data[this.columns.sessionId] = this.getsid();
            //浏览器分辨率
            data[this.columns.resolution] = window.screen.width + "*" + window.screen.height;
            //客户端语言
            data[this.columns.language] = window.navigator.language;
            //客户端当前时间
            data[this.columns.clientTime] = new Date().getTime();
        },

        /**
         * 生成唯一标识guid
         */
        guid: function () {
            return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
                var r = Math.random() * 16 | 0, v = c == 'x' ? r : (r & 0x3 | 0x8);
                return v.toString(16);
            });
        }

    };
    tarck.sessionStart();


})();